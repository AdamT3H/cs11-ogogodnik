        ...
        Array(halfCount)
          ...
          .map((_el, index) => {
            ...
            return (<div
              ...
              style={{ 'color': allDatesStr.includes(showYear + 1 + "") 
              ? 'green' 
              : 'white' }}
            >{showYear + 1}</div>);
          })
      ...
      <div className='year content-item' onClick={() => click(year)} >{year}</div>
      {
        Array(halfCount)
          ...
          .map((_el, index) => {
            ...
            const allDatesStr = datesWithEvents.map(date => date.year)
            return (
              <div style={{ 'color': allDatesStr.includes(showYear + 1 + "") ? 'green' : 'white' }} className='year content-item' onClick={() => click(showYear)} >
                {showYear}
              </div>);
          })
      }
